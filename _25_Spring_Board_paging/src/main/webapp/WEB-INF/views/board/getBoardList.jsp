		...
		<table id="boardTable" border="1"
			...
			<c:forEach items="${boardList }" var="board">
				<tr>
					<td>${board.boardNo }</td>
					<td><a
						href="/board/updateBoardCnt.do?boardNo=${board.boardNo }">
		...
		<div style="text-align: centet;">
			<ul class="pagination">
				<c:if test="${pageDTO.prev }">
					<li class="pagination-btn"><a
						href="${pageDTO.cri.pageNum - 1} }">이전</a></li>
				</c:if>
				<c:forEach begin="${pageDTO.startPage }" end="${pageDTO.endPage }"
					var="num">
					<li class="pagination-btn"><a href="${num }">${num }</a></li>
				</c:forEach>

				<c:if test="${pageDTO.next }">
					<li class="pagination-btn"><a
						href="${pageDTO.cri.pageNum + 1} }">다음</a></li>
				</c:if>
